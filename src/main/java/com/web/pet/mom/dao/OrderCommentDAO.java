...
public class OrderCommentDAO {
    ...
    @Autowired
    public OrderCommentDAO(SessionFactory sessionFactory) {
        this.sessionFactory = sessionFactory;
    }

    public void insertOrderComment(OrderComment raing) { //新增評論
        Session session = sessionFactory.getCurrentSession();
        session.save(raing);
    }


    public List<OrderComment> searchOrderComment(int uId) { //查詢一筆評論
    ...
    public int updateOrderComment(OrderComment orderComment) {//修改評論
        int count = 0;
        Session session = sessionFactory.getCurrentSession();
    ...
    public int deleteOrderComment(OrderComment orderComment) {//刪除評論
        int count = 0;
        Session session = sessionFactory.getCurrentSession();
